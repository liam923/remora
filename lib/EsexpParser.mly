...
esexp:
  | left = tokenSource(LEFT_PAREN); elements = esexp*; right = tokenSource(RIGHT_PAREN)   { Esexp.List {braceType=Paren; elements; braceSources=(left, right)}  }
  | left = tokenSource(LEFT_SQUARE); elements = esexp*; right = tokenSource(RIGHT_SQUARE) { Esexp.List {braceType=Square; elements; braceSources=(left, right)}  }
